...
for key in transit:
	...
	if header != None: 
		...
		#### Bus line ###########################
		#########################################
		#query = """DELETE FROM routes WHERE name = "%s" AND IO = "%s" AND time = "%s";""" % (header,d,s)
		#cur.execute(query) 	
		query = """INSERT INTO routes (name,IO,time) VALUES ("%s","%s","%s")""" % (header,d,s)
		cur.execute(query) 		
		...
		for bus in sch:
			#query = """DELETE FROM buses WHERE name = "%s";""" % (bus)
			#cur.execute(query)	
			query = """INSERT INTO buses (routeID,name) VALUES (%s,"%s")""" % (routeID,bus)
			cur.execute(query) 		
			busID = cur.lastrowid
			...
			for stop in sch[bus]:
				if stop != "--":
					...
					query = """INSERT INTO stops (busID,time) VALUES (%s,"%s")""" % (busID,t24)
					cur.execute(query) 		
